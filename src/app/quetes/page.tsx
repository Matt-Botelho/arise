...
export default function QuetesPage() {
  ...
  async function load() {
    ...
    setQuests(r.quests || []); setDay(r.day || ""); setLoading(false);
  }
  useEffect(() => { load(); }, []);
  ...
  async function complete(id: string) {
    ...
    if (r.promoted) msg = "⩘ RANG " + r.promoted.to + " ATTEINT !";
  ...
  return (
    <div className="space-y-4">
      ...
      {rankup.length > 0 && <SystemPanel title="[ Épreuve de promotion ]">{rankup.map((q) => <Row key={q.id} q={q} onDone={complete} />)}</SystemPanel>}
