...
type DomState = { court: string; moyen: string; sel: number[] };

export default function OnboardingPage() {
  const [name, setName] = useState("");
  const [dom, setDom] = useState<Record<string, DomState>>(
    Object.fromEntries(ATTRIBUTES.map((a) => [a.code, { court: "", moyen: "", sel: [] }]))
  );
  ...
  async function finish() {
    setSaving(true);
    const objectives: { attributeCode: string; horizon: string; title: string; quests?: { title: string; baseXp: number; difficulty: string }[] }[] = [];
    for (const a of ATTRIBUTES) {
      const d = dom[a.code];
      if (d.court.trim()) {
        const quests = d.sel.map((i) => (SUGGESTIONS[a.code] || [])[i]).filter(Boolean).map((t) => ({ title: t.title, baseXp: t.baseXp, difficulty: t.difficulty }));
        objectives.push({ attributeCode: a.code, horizon: "court", title: d.court.trim(), quests });
      }
      if (d.moyen.trim()) objectives.push({ attributeCode: a.code, horizon: "moyen", title: d.moyen.trim() });
    }
    if (objectives.length === 0) { setMsg("Définis au moins un objectif."); setSaving(false); return; }
  ...
  const nbCourt = ATTRIBUTES.filter((a) => dom[a.code].court.trim()).length;

  return (
    <div className="space-y-4">
      <h1 className="text-lg uppercase tracking-[0.2em] text-system-accent system-glow">Onboarding du Chasseur</h1>
      <p className="text-sm text-system-text/70">Définis tes objectifs <b>court terme</b> (ce mois) et <b>moyen terme</b> (3 mois) pour chaque domaine. Coche les quêtes suggérées que tu veux activer. Tu pourras tout ajuster ensuite.</p>
      ...
      {ATTRIBUTES.map((a) => {
        ...
        return (
          <SystemPanel key={a.code} title={"[ " + a.icon + " " + a.name + " ]"}>
            ...
            {sug.length > 0 && (
              <>
                <p className="mt-3 text-xs uppercase tracking-widest text-system-text/60">Quêtes journalières suggérées</p>
