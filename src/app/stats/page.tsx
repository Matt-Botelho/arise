  ...
  return (
    <div className="space-y-4">
      ...
      <SystemPanel title="[ Titre équipé ]">
        ...
        <p className="mt-1 text-xs text-system-text/40">S'affiche sur ta fiche de Chasseur (onglet Statut).</p>
        {titles.length > 0 && (
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {items.map((a) => (
                <div key={a.key} className="rounded border p-2 text-center" style={{ borderColor: a.unlocked ? TIER_COLOR[tier] : "rgba(31,111,235,0.2)", opacity: a.unlocked ? 1 : 0.45 }}>
                  ...
                  <div className="text-[11px] text-system-text/50">{a.description}</div>
                  ...
                  {a.reward.skin && (
                    <div className="mt-1 flex flex-col items-center">
                      ...
                      <span className="text-[10px]" style={{ color: TIER_COLOR[tier] }}>skin exclusif</span>
                    </div>
                  )}
                </div>
              ))}
