...
export function rollLoot(owned: string[], difficulty: string, rnd: () => number = Math.random, chanceBonus = 0): LpcItem | null {
  ...
  const pool = ITEMS.filter((i) => i.rarity !== "commun" && i.rarity !== "base" && !COSMETIC_SET.has(i.key) && !ACHIEVEMENT_SKINS.has(i.key));
...
export function pickRankLoot(owned: string[]): LpcItem | null {
  ...
  for (const rar of ["legendaire", "epique", "rare"]) {
    const unowned = ITEMS.filter((i) => i.rarity === rar && !have.has(i.key) && !COSMETIC_SET.has(i.key) && !ACHIEVEMENT_SKINS.has(i.key));
    if (unowned.length) return unowned[Math.floor(Math.random() * unowned.length)];
  }
  for (const rar of ["legendaire", "epique", "rare"]) {
    const any = ITEMS.filter((i) => i.rarity === rar && !COSMETIC_SET.has(i.key) && !ACHIEVEMENT_SKINS.has(i.key));
